...
from .schemas import schema_set_hash, validate
...
def load(paths: Paths) -> dict[str, Any]:
    ...
    if record["schema_set_hash"] != schema_set_hash():
        raise DomainError(
            ["schema set drift: this session was created under a different frozen "
             "schema set (P5). Migrate explicitly; nd will not guess."])
    return record
