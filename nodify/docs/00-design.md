# Nodify — 设计 V1(revision 1, 2026-07-09)

> **一句话**:把大模型深度搜索的产出从超长 markdown 上下文里解放出来——
> 逻辑与论据被持续抽取进一棵持久化的 node 树,原始文字阅后即焚。
>
> 权威顺序:`nodify/schemas/*.schema.json`(V1 冻结后)> 测试 > 本文(导读)。
...
3. `node.v1` — {node_id N-0001, parent_id?, kind, statement, why_helps_parent?,
   orientation ∈ {neutral, adversarial}?, status, promotion_note?, stuck_reason?,
   revises?, created_at, created_by}
   (kind↔status 合法组合由 schema 的条件分支强制)
...
每条命令一个 envelope。清单由契约测试双向镜像;扩表 = 先改本文再改测试。
...
1. **蒸馏义务**:每轮搜索/子调查结束,先 `nd add / conclude` 再继续;原始
   文本进 `notes/` 或直接丢弃;禁止把搜索原文堆进对话继续滚。
...
| P4 反过度机器 | hard(9 条 CLI + 4 schema + 无队列/锁/状态机引擎) |
...
| P1/P3 契约与接线 | 缩面后 hard(4 schema 各有产/销/贯穿测试) |
...
- **M0 冻结**:三场景推演(论文调研 / 技术选型 / bug 根因)对着 4 schema 走
