...
**19 条 claim（含 6 条真实中文文献），0 条达到 `verified`。** 最高只到 `attributed`——因为六份真实文献
没有一份给得出 G5（需要 JATS/TEI 结构化锚，那要真正的 API 集成，我们没有）。


## T4 · 真实中文文献（S9）

`13 条 / 6 篇 / 3 话题` 这条覆盖面缺口，只能靠加真实文献补，加门补不了。
T4 是第一步：三份 Europe PMC 上的**中文**摘要（`LANG:"chi"`）。

选它们是因为系统里有一整套 CJK 处理——归一化、中文否定表、中文数字——
**此前只在作者自己造的句子上验过**，那正是 SA-3 那条循环性。

| 用例 | 压的是什么 | 结果 |
|---|---|---|
| T4-1 | 全角数字 `73．55％`（全角句点 U+FF0E + 全角百分号 U+FF05），载荷写半角 | attributed |
| T4-2 | 同句两个并列读数（73．55％ 与 78．45％），未声明取哪一个 | unverified |
| T4-3 | 中文空结果「无显著相关性」被当成支持证据 | unverified |
| T4-4 | 中文上界限定「小于2%」被转录成「差值为 2%」 | unverified |
| T4-5 | 「但对SPH…没有显著影响」被取来支持「移栽期对 SPH 有影响」 | unverified |
| T4-6 | 同句「但」**之前**的肯定发现，不得误伤 | attributed |

它找到的四条：

- **`numericForm` 没做 NFKC**。组稿器那边早就在扫描前 NFKC 了（E-3），
  锚点包含这边漏了——**同一个坑挖了两次**。半角载荷对不上全角原文，
  一条完全合法的转录被判成假。
- **中文空结果表述同样把主语放在算子之前**。「没有」是 NEG-S，作用域只往后延，
  够不着它前面的 SPH。与英文那一批同理，归入 NEG-N 取子句作用域。
- **G-FRAME 挑错了子句**。原实现取「第一个是锚句子串的子句」，
  于是选中 `分别为100％` 这种与载荷无关的片段，判红的**理由完全是错的**。
  一道理由错了的红，和放行一样坏：它把人引去改一个没坏的地方。
- **我的夹具 `sentenceWith` 不认中文句号**，把两句当成一句，
  前一句的否定打中了后一句的载荷。那是夹具造出来的假阳，不是门的。
  修它时我又加错了一个小数点守卫（查的是句号后**第二个**字符），
  导致 `results; 47%` 里的分号不算句末，T2-2 与 T3-6 回归。
  **守卫写在错的位置，比没有守卫更难查。**

另外 G-FRAME 在一条**我没为它设计的用例**上第一次触发：
Wouters 那句同时给了中位数 $985.3M 与均值 $1335.9M，我写用例时只看见中位数，
是门告诉我这句需要 discriminator 的。顺着它又查出 ` and ` 的切分问题——
