...
- `2026-08-17` · **R3 fix-audit 回来了：92 条 findings（P1 31）**。三条最重的 P1 已修并配回归用例。
  其中 R3/P1-2 打穿了我们自己的头号证据：穷举 oracle 把 `gate_class` 硬编码成 GC-0，
  因此「GC-2 永不得写 ST-V」从未被枚举到——**550 万向量全绿与产品最承重的分界线完全无关**。
  oracle 已扩充，向量数 5,557,248 → **13,893,120**。
  **R3 未收敛，R4 是必需的**：92 条只处理了 3 条，且 `gate-hollow` 轴独占 21 条。
...
| 阶段 | 状态 | 产出 |
|---|---|---|
| S0 阻塞项实测 | 完成 | 12 份记录，26/26 证据全部重跑且哈希一致 |
| S1 规范源 + S 的可执行副本 | 完成 | 82 条黄金用例、2778 万向量 oracle、31 个变异体 |
| **S2 产品骨架** | **完成** | profile（实测可跑）、写者契约、§1.2.2 双实现、L1-c、端到端管线 |
| **S3 证据引擎** | **完成** | G-CLUSTER、G-CTR-SCAN、CAS + 证据卡、组稿器、抓取锚点、全链路 |
| S4 编排（多 loop 并行探索） | 未开始 | — |

### 门（20 道，全绿）

规范层 6 道 · 产品层 11 道 · 负例套件 4 套（M0 19 条 / 指针 12 条 / 写者契约 7 条 / 规范源自洽）

标定集：L1-c 11 条（假阴 6 / 假阳 5）· G-CLUSTER 9 条（压塌 5 / 独立 3 / 低置信 1）·
X-2 10 条（假阳 4 / 假阴 6）· 组稿器 9 条 · 端到端 13 条 · 全链路 4 类 · CAS 3 类 · 锚点 7+4 类

### 核心承诺的可执行形式

> **从抓取到成稿，中间没有任何一步允许 agent 直接写结论。**

`gates/check_full_chain.mjs` 是这句话的验收：正常链路 → 成稿带 ST-V；篡改 CAS 快照 → 状态降级；
5 种绕过尝试（producer 写 status / 自报谓词 / 作者敲裸数字 / 非 200 当快照 / 证据卡缺分量）
全部在链路上被拒；没做反证检索 → `not_covered`。

### 本轮的方法论收获

1. **先做软肋，不是先做业务逻辑。** R5 预测「六个自报谓词是 fail-closed 唯一的出口」，
   于是产品第一块代码是写者契约，且它的门守的是**结构不变式**
   （∀ S 读的字段：有主 ∧ ≠ producer），读集从源码自动提取。
2. **单一实现的自洽不是证据。** §1.2.2 有 JS + Python 两个独立实现互为对拍，
   且用例从已冻结的 S0 结论那侧提取。
3. **标定集必须两侧都有样本。** 只测一侧的门会朝那一侧漂，
   而 R5 预测的两个失败恰好是同一条轴的两端。
4. **自攻同样要先跑再写。** S3 自攻 6 条里唯一一条先写后跑的（SA-6）是唯一一条错的，已撤回。

### 待办（诚实清单）

- R6 攻击进行中（目标已按 `.attack/r6-target-hashes.json` 冻结 —— 这是对 R4/R5 两轮
  「审计期间目标移动」那条方法论错误的纠正）
- 自攻已登记 5 条待修（`.attack/self-attack-s3.json`）：L1-c 词表 8 个假阴 + 双重否定假阳、
  组稿器的中文/全角数字绕过、G-CLUSTER 的自陈依赖
- R5 第 1 条预测（V1.2 与 §8.6.2.1 的冲突）仍**没有任何门能看见**
- S4 编排层（超大并行多 loop 论据探索）未开始 —— 这是用户原始需求的第 3 条
