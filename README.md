...
**当前状态：规划 + 七轮攻击 + 产品层跑通 + 外部标定测试 + 结构化抓取（ST-V 可达）。**

> **同一句话，两种取证方式，成稿里的状态标记不同**——这是这个系统的产品：
> ```
> PDF 抽取（无稳定锚，G4）  → AlphaFold 的中位主链精度为 0.96〔已归因〕Å。
> JATS 结构化（jats:Sec1/Par5，回指往返已验，G5）
>                          → AlphaFold 的中位主链精度为 0.96〔已验证〕Å。
> ```
> 同一篇论文、同一个数字、同一条引语。差别只在**能不能被独立重新寻址**。

`./gates/run_all.sh all` 当前 **35/35 全绿**，退出码判定。产品代码约 3900 行，门与外部测试另有约 7000 行：
...
| **取证** | `packages/dsh-academic-fetch`（Europe PMC JATS · arXiv LaTeXML HTML） | 解析与定位互逆、回指往返四情形、G5 只给验过的 |
| **成本** | `src/cost.mjs`（DeepSeek v4 计量，按阶段/按 status 归集） | 7 条手算算例 + 9 个峰时端点 + 分项之和 = 总额 |
...
| [07-ATTACK-LEDGER.md](07-ATTACK-LEDGER.md) | **攻击台账**——每轮的靶标指纹、种子命中率、findings 全量、裁决与修复状态 | 2358 行 / R1 收 164 条 |
