...
@app.command()
def reparent(node_id: str = typer.Argument(...),
             to: str = typer.Option(..., "--to", help="new parent node id"),
             note: Optional[str] = typer.Option(None, "--note"),
             root: Optional[str] = _ROOT_OPT, session: Optional[str] = _SESSION_OPT) -> None:
    def go(paths: Paths, _sess):
        record = tree.reparent(paths, node_id, to, note=note)
        return {"node": record}, [node_id, to], f"reparent {node_id} -> {to}"
    _run("reparent", root, session, go, mutating=True)


@app.command()
def reframe(node_id: str = typer.Argument(...),
            statement: str = typer.Option(..., "--statement",
                                          help="reframed statement, updated in place"),
            note: Optional[str] = typer.Option(None, "--note"),
            root: Optional[str] = _ROOT_OPT, session: Optional[str] = _SESSION_OPT) -> None:
    def go(paths: Paths, _sess):
        record = tree.reframe(paths, node_id, statement, note=note)
        return {"node": record}, [node_id], f"reframe {node_id}"
    _run("reframe", root, session, go, mutating=True)
