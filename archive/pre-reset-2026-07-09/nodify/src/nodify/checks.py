...
def run(paths: Paths, session: dict[str, Any]) -> tuple[list[str], list[str]]:
    ...
    # Budgets are advisory guardrails, not gates (loosened v0.2): surface sprawl as
    # SOFT warnings so the model can SEE it's going wide/deep, but nothing blocks —
    # the model, not the CLI, decides when the shape is right.
    if not hard:  # depth walk needs intact parents
        for n in nodes.values():
            if tree.depth_of(nodes, n["node_id"]) > budgets["max_depth"]:
                soft.append(f"{n['node_id']}: depth over guardrail "
                            f"max_depth={budgets['max_depth']}")
    for n in nodes.values():
        kids = tree.children_of(nodes, n["node_id"])
        if len(kids) > budgets["max_children"]:
            soft.append(f"{n['node_id']}: {len(kids)} children over guardrail "
                        f"max_children={budgets['max_children']}")
    open_claims = tree._open_claims(nodes)
    if open_claims > budgets["max_open_claims"]:
        soft.append(f"{open_claims} open claims over guardrail "
        ...
        if n["status"] in ACTIVE:
            ...
            while cur["parent_id"] in nodes:
                ...
                if cur["status"] in DEAD_ANCESTOR:
                    ...
                    break
    # --- soft: DEPTH + RIGOR scaffold (docs/12) — make satisficing + unverified
    # conclusions visible. The agent (via the deep-rigor skill) clears these; the
    # framework only surfaces them. Reuses existing records, no schema change.
    CONCLUDED_STATES = {"synthesized", "concluded", "closed"}
    latest_syn = {}
    for s in syn_records:
        latest_syn[s["node_id"]] = s  # append order → last wins
    for n in nodes.values():
        if n["status"] not in CONCLUDED_STATES:
            continue
        kids = tree.children_of(nodes, n["node_id"])
        # RIGOR gate: a conclusion that rolls up children must have survived a
        # RESOLVED red-team (an adversarial child that was itself concluded) —
        # keep-if-survives, not keep-if-plausible.
        if kids and not any(k["orientation"] == "adversarial"
                            and k["status"] in CONCLUDED_STATES for k in kids):
            soft.append(f"{n['node_id']}: conclusion not red-teamed — no resolved "
                        "adversarial challenge (verify it or refute it)")
        # DEPTH push: a non-root conclusion that raised deeper open_questions but
        # spawned no child to pursue them satisficed early (went shallow).
        syn = latest_syn.get(n["node_id"])
        if n["parent_id"] is not None and syn and syn.get("open_questions") and not kids:
            soft.append(f"{n['node_id']}: shallow — raised "
                        f"{len(syn['open_questions'])} deeper question(s) but pursued none")
